## nanoeigenpy (humble) - 0.5.0-1

The packages in the `nanoeigenpy` repository were released into the `humble` distro by running `/nix/store/rhf364g29a4h0nwdhf8lv17y9jh0i5gj-python3.13-bloom-0.13.0/bin/bloom-release -r humble nanoeigenpy` on `Fri, 27 Mar 2026 13:58:41 -0000`

The `nanoeigenpy` package was released.

Version of package(s) in repository `nanoeigenpy`:

- upstream repository: https://github.com/Simple-Robotics/nanoeigenpy.git
- release repository: https://github.com/ros2-gbp/nanoeigenpy-release.git
- rosdistro version: `0.4.0-1`
- old version: `0.4.0-1`
- new version: `0.5.0-1`

Versions of tools used:

- bloom version: `0.13.0`
- catkin_pkg version: `1.1.0`
- rosdep version: `0.25.1`
- rosdistro version: `1.0.1`
- vcstools version: `0.1.42`


## nanoeigenpy (jazzy) - 0.5.0-1

The packages in the `nanoeigenpy` repository were released into the `jazzy` distro by running `/nix/store/rhf364g29a4h0nwdhf8lv17y9jh0i5gj-python3.13-bloom-0.13.0/bin/bloom-release -r jazzy nanoeigenpy` on `Fri, 27 Mar 2026 13:56:35 -0000`

The `nanoeigenpy` package was released.

Version of package(s) in repository `nanoeigenpy`:

- upstream repository: https://github.com/Simple-Robotics/nanoeigenpy.git
- release repository: https://github.com/ros2-gbp/nanoeigenpy-release.git
- rosdistro version: `0.4.0-1`
- old version: `0.4.0-1`
- new version: `0.5.0-1`

Versions of tools used:

- bloom version: `0.13.0`
- catkin_pkg version: `1.1.0`
- rosdep version: `0.25.1`
- rosdistro version: `1.0.1`
- vcstools version: `0.1.42`


## nanoeigenpy (kilted) - 0.5.0-1

The packages in the `nanoeigenpy` repository were released into the `kilted` distro by running `/nix/store/rhf364g29a4h0nwdhf8lv17y9jh0i5gj-python3.13-bloom-0.13.0/bin/bloom-release -r kilted nanoeigenpy` on `Fri, 27 Mar 2026 13:53:50 -0000`

The `nanoeigenpy` package was released.

Version of package(s) in repository `nanoeigenpy`:

- upstream repository: https://github.com/Simple-Robotics/nanoeigenpy.git
- release repository: https://github.com/ros2-gbp/nanoeigenpy-release.git
- rosdistro version: `0.4.0-1`
- old version: `0.4.0-1`
- new version: `0.5.0-1`

Versions of tools used:

- bloom version: `0.13.0`
- catkin_pkg version: `1.1.0`
- rosdep version: `0.25.1`
- rosdistro version: `1.0.1`
- vcstools version: `0.1.42`


## nanoeigenpy (rolling) - 0.5.0-1

The packages in the `nanoeigenpy` repository were released into the `rolling` distro by running `/nix/store/rhf364g29a4h0nwdhf8lv17y9jh0i5gj-python3.13-bloom-0.13.0/bin/bloom-release -r rolling nanoeigenpy` on `Fri, 27 Mar 2026 13:51:49 -0000`

The `nanoeigenpy` package was released.

Version of package(s) in repository `nanoeigenpy`:

- upstream repository: https://github.com/Simple-Robotics/nanoeigenpy.git
- release repository: https://github.com/ros2-gbp/nanoeigenpy-release.git
- rosdistro version: `0.4.0-1`
- old version: `0.4.0-1`
- new version: `0.5.0-1`

Versions of tools used:

- bloom version: `0.13.0`
- catkin_pkg version: `1.1.0`
- rosdep version: `0.25.1`
- rosdistro version: `1.0.1`
- vcstools version: `0.1.42`


## nanoeigenpy (jazzy) - 0.4.0-1

The packages in the `nanoeigenpy` repository were released into the `jazzy` distro by running `/nix/store/rrnp44mx5czg1688ynsgf6aibjf0zcq2-python3.12-bloom-0.12.0/bin/bloom-release -r jazzy nanoeigenpy` on `Tue, 14 Oct 2025 15:38:58 -0000`

The `nanoeigenpy` package was released.

Version of package(s) in repository `nanoeigenpy`:

- upstream repository: https://github.com/Simple-Robotics/nanoeigenpy.git
- release repository: https://github.com/ros2-gbp/nanoeigenpy-release.git
- rosdistro version: `0.3.0-1`
- old version: `0.3.0-1`
- new version: `0.4.0-1`

Versions of tools used:

- bloom version: `0.12.0`
- catkin_pkg version: `1.0.0`
- rosdep version: `0.25.1`
- rosdistro version: `0.9.1`
- vcstools version: `0.1.42`


## nanoeigenpy (rolling) - 0.4.0-1

The packages in the `nanoeigenpy` repository were released into the `rolling` distro by running `/nix/store/rrnp44mx5czg1688ynsgf6aibjf0zcq2-python3.12-bloom-0.12.0/bin/bloom-release -r rolling nanoeigenpy` on `Tue, 14 Oct 2025 14:43:06 -0000`

The `nanoeigenpy` package was released.

Version of package(s) in repository `nanoeigenpy`:

- upstream repository: https://github.com/Simple-Robotics/nanoeigenpy.git
- release repository: https://github.com/ros2-gbp/nanoeigenpy-release.git
- rosdistro version: `0.3.0-1`
- old version: `0.3.0-1`
- new version: `0.4.0-1`

Versions of tools used:

- bloom version: `0.12.0`
- catkin_pkg version: `1.0.0`
- rosdep version: `0.25.1`
- rosdistro version: `0.9.1`
- vcstools version: `0.1.42`


## nanoeigenpy (kilted) - 0.4.0-1

The packages in the `nanoeigenpy` repository were released into the `kilted` distro by running `/nix/store/rrnp44mx5czg1688ynsgf6aibjf0zcq2-python3.12-bloom-0.12.0/bin/bloom-release -r kilted nanoeigenpy` on `Tue, 14 Oct 2025 14:30:30 -0000`

The `nanoeigenpy` package was released.

Version of package(s) in repository `nanoeigenpy`:

- upstream repository: https://github.com/Simple-Robotics/nanoeigenpy.git
- release repository: https://github.com/ros2-gbp/nanoeigenpy-release.git
- rosdistro version: `0.3.0-1`
- old version: `0.3.0-1`
- new version: `0.4.0-1`

Versions of tools used:

- bloom version: `0.12.0`
- catkin_pkg version: `1.0.0`
- rosdep version: `0.25.1`
- rosdistro version: `0.9.1`
- vcstools version: `0.1.42`


## nanoeigenpy (humble) - 0.4.0-1

The packages in the `nanoeigenpy` repository were released into the `humble` distro by running `/nix/store/rrnp44mx5czg1688ynsgf6aibjf0zcq2-python3.12-bloom-0.12.0/bin/bloom-release -r humble nanoeigenpy` on `Tue, 14 Oct 2025 14:15:52 -0000`

The `nanoeigenpy` package was released.

Version of package(s) in repository `nanoeigenpy`:

- upstream repository: https://github.com/Simple-Robotics/nanoeigenpy.git
- release repository: https://github.com/ros2-gbp/nanoeigenpy-release.git
- rosdistro version: `0.3.0-1`
- old version: `0.3.0-1`
- new version: `0.4.0-1`

Versions of tools used:

- bloom version: `0.12.0`
- catkin_pkg version: `1.0.0`
- rosdep version: `0.25.1`
- rosdistro version: `0.9.1`
- vcstools version: `0.1.42`


## nanoeigenpy (rolling) - 0.3.0-1

The packages in the `nanoeigenpy` repository were released into the `rolling` distro by running `/nix/store/rrnp44mx5czg1688ynsgf6aibjf0zcq2-python3.12-bloom-0.12.0/bin/bloom-release -r rolling nanoeigenpy` on `Wed, 09 Jul 2025 15:20:33 -0000`

The `nanoeigenpy` package was released.

Version of package(s) in repository `nanoeigenpy`:

- upstream repository: https://github.com/Simple-Robotics/nanoeigenpy.git
- release repository: unknown
- rosdistro version: `null`
- old version: `null`
- new version: `0.3.0-1`

Versions of tools used:

- bloom version: `0.12.0`
- catkin_pkg version: `1.0.0`
- rosdep version: `0.25.1`
- rosdistro version: `0.9.1`
- vcstools version: `0.1.42`


## nanoeigenpy (kilted) - 0.3.0-1

The packages in the `nanoeigenpy` repository were released into the `kilted` distro by running `/nix/store/rrnp44mx5czg1688ynsgf6aibjf0zcq2-python3.12-bloom-0.12.0/bin/bloom-release -r kilted nanoeigenpy` on `Wed, 09 Jul 2025 15:18:09 -0000`

The `nanoeigenpy` package was released.

Version of package(s) in repository `nanoeigenpy`:

- upstream repository: https://github.com/Simple-Robotics/nanoeigenpy.git
- release repository: unknown
- rosdistro version: `null`
- old version: `null`
- new version: `0.3.0-1`

Versions of tools used:

- bloom version: `0.12.0`
- catkin_pkg version: `1.0.0`
- rosdep version: `0.25.1`
- rosdistro version: `0.9.1`
- vcstools version: `0.1.42`


## nanoeigenpy (jazzy) - 0.3.0-1

The packages in the `nanoeigenpy` repository were released into the `jazzy` distro by running `/nix/store/rrnp44mx5czg1688ynsgf6aibjf0zcq2-python3.12-bloom-0.12.0/bin/bloom-release -r jazzy nanoeigenpy` on `Wed, 09 Jul 2025 15:15:26 -0000`

The `nanoeigenpy` package was released.

Version of package(s) in repository `nanoeigenpy`:

- upstream repository: https://github.com/Simple-Robotics/nanoeigenpy.git
- release repository: unknown
- rosdistro version: `null`
- old version: `null`
- new version: `0.3.0-1`

Versions of tools used:

- bloom version: `0.12.0`
- catkin_pkg version: `1.0.0`
- rosdep version: `0.25.1`
- rosdistro version: `0.9.1`
- vcstools version: `0.1.42`


## nanoeigenpy (humble) - 0.3.0-1

The packages in the `nanoeigenpy` repository were released into the `humble` distro by running `/nix/store/rrnp44mx5czg1688ynsgf6aibjf0zcq2-python3.12-bloom-0.12.0/bin/bloom-release -r humble nanoeigenpy` on `Wed, 09 Jul 2025 15:09:29 -0000`

The `nanoeigenpy` package was released.

Version of package(s) in repository `nanoeigenpy`:

- upstream repository: https://github.com/Simple-Robotics/nanoeigenpy.git
- release repository: unknown
- rosdistro version: `null`
- old version: `null`
- new version: `0.3.0-1`

Versions of tools used:

- bloom version: `0.12.0`
- catkin_pkg version: `1.0.0`
- rosdep version: `0.25.1`
- rosdistro version: `0.9.1`
- vcstools version: `0.1.42`


